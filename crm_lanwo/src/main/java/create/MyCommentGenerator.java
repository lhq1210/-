 ...
 *
 * @author wzz
 * @version 创建时间：2019/5/21 17:32:30
 */
...
//        field.addJavaDocLine(" */");
        ...
        /*List<IntrospectedColumn> keyColumns = introspectedTable.getBaseColumns();
        for (IntrospectedColumn col : keyColumns) {
            if (col.getJdbcTypeName().equals("TIMESTAMP")) {
                field.addAnnotation("@JSONField(format = \"yyyy-MM-dd HH:mm:ss\")");
            }
        }*/
        //属性前加注解信息
        if (introspectedColumn.getJdbcTypeName().equals("TIMESTAMP")) {
            field.addAnnotation("@ApiModelProperty(value = \"" + introspectedColumn.getRemarks() + "\", example = \"2018-10-01 12:18:48\")");
            field.addAnnotation("@JSONField(format = \"yyyy-MM-dd HH:mm:ss\")");
        } else {
